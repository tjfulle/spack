...
def add_command(parser, command_dict):
    pymod_parser = parser.add_parser(
        'pymod', help='manipulate hierarchical module files'
    )
    sp = spack.cmd.modules.setup_parser(pymod_parser)
    ...
    command_dict['pymod'] = functools.partial(
        spack.cmd.modules.modules_cmd, module_type='pymod', callbacks=callbacks
    )
...
def setdefault(module_type, specs, args):
    ...
    writer = spack.modules.module_types['pymod'](specs[0])
