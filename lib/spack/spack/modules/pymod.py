...
#: pymod specific part of the configuration
def configuration():
    return spack.config.get('modules:pymod', {})
...
def make_configuration(spec):
    """Returns the pymod configuration for spec"""
    key = spec.dag_hash()
    try:
        return configuration_registry[key]
    except KeyError:
        return configuration_registry.setdefault(key, PymodConfiguration(spec))


def make_layout(spec):
    """Returns the layout information for spec """
    conf = make_configuration(spec)
    return PymodFileLayout(conf)


def make_context(spec):
    """Returns the context information for spec"""
    conf = make_configuration(spec)
    return PymodContext(conf)
...
def guess_core_compilers(store=False):
    ...
    if store and core_compilers:
        ...
        modules_cfg.setdefault('pymod', {})['core_compilers'] = core_compilers
        spack.config.set(
...
class PymodConfiguration(BaseConfiguration):
    """Configuration class for pymod module files."""
    ...
    def hierarchy_tokens(self):
        ...
        if not_virtual:
            msg = "Non-virtual specs in 'hierarchy' list for pymod: {0}\n"
...
class PymodFileLayout(BaseFileLayout):
    """File layout for pymod module files."""

    #: file extension of pymod module files
    extension = 'py'
    ...
    def arch_dirname(self):
        ...
        return os.path.join(
            self.dirname(),  # root for pymod module files
    ...
    def filename(self):
        ...
        fullname = os.path.join(
            self.arch_dirname,  # root for pymod files on this architecture
...
class PymodContext(BaseContext):
    """Context class for pymod module files."""
...
class PymodModulefileWriter(BaseModuleFileWriter):
    """Writer class for pymod module files."""
    default_template = os.path.join('modules', 'modulefile.py')
...
class NonVirtualInHierarchyError(spack.error.SpackError, TypeError):
    ...
    the pymod section of 'modules.yaml'.
    """
