...
whatis(configure_options="{{ configure_options }}")
{% endif %}
...
{% for name in missing %}
{{ name }}_name = os.getenv("PYMOD_{{ name|upper() }}_NAME")
{{ name }}_version = os.getenv("PYMOD_{{ name|upper() }}_VERSION")
{% endfor %}
...
{% for name in provides %}
setenv("PYMOD_{{ name|upper() }}_NAME", "{{ name_part }}")
setenv("PYMOD_{{ name|upper() }}_VERSION", "{{ version_part }}")
{% endfor %}
{% endif %}
{% endif %}
{% endblock %}
...
{% if verbose %}
    log_message("Autoloading {{ module }}")
{% endif %}
    load("{{ module }}")
{% endfor %}
{% endblock %}
...
{% block footer %}
{# In case the module needs to be extended with custom Python code #}
{% endblock %}
